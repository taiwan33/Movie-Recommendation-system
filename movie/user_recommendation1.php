...
while($movie=mysqli_fetch_array($movies)){
    ...
    $users=mysqli_query($con, "SELECT username,above_14 from login_form where id=$movie[user_id]");
...
if('above_14'==0){
    $allMoviesResult = $allMoviesResult. " AND (movies_genres LIKE '%Adventure%' OR movies_genres LIKE '%Science Fiction%')";
  }

$recommendedMoviesDetails = array();
foreach($recommendedMovies as $key=>$value){
    $recommendedMoviesDetails[] = $allMovies[array_search($key, array_column($allMovies,'movie_name'))];
}
...
foreach($recommendedMoviesDetails as $movie){
    ...
    ?>
    Homepage: <a target="_blank" href="<?php echo $movie["homepage"] ?>"><?php echo $movie["homepage"] ?></a><br>
    <?php
    ...
    echo "Official Home page: ".$movie["homepage"]."<br/><br/><br/>";
}
...
<!-- Homepage: <a target="_blank" href="<?php echo $row["homepage"] ?>"><?php echo $row["homepage"] ?></a><br> -->